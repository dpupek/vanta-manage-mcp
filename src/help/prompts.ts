...
const controlEvidencePrompt = (
  ...
  const text = [
    ...
    "3. If uploading file evidence, use `upload_file_for_document` with `filePath` and optional `mimeType`.",
...
const vulnerabilityDueSoonPrompt = (
  ...
  const text = [
    ...
    "3. Optimization for `top vulnerable devices` requests: rank assets first from `list_vulnerable_assets`, then call `list_vulnerabilities` only for selected `vulnerableAssetId` values.",
    "4. Cross-reference scanner metadata with `integration_resources` for Defender/connected source details.",
    "5. Rank by severity + due date + critical asset exposure.",
    "6. For approved actions, run `workflow_people_assets_vuln_triage` in `mode=plan` first, then `mode=execute` + `confirm=true`.",
    "7. Verify with readback of vulnerabilities, remediations, and affected assets.",
    "",
    executionReminder,
  ].join("\n");
